...
export async function handleAnchors(request: Request, env: Env, path: string): Promise<Response> {
  ...
  if (sourceMatch && method === 'GET') {
    ...
    const row = await db.prepare('SELECT * FROM watcher_status WHERE source = ?').bind(source).first() as WatcherRow | null;
  ...
  if (path.startsWith('/api/anchors/') && method === 'PUT') {
    ...
    const existing = await db.prepare('SELECT * FROM watcher_status WHERE source = ?').bind(source).first() as WatcherRow | null;
    if (!existing) {
