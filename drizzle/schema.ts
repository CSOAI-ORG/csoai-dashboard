...
export const councilVotes = mysqlTable("council_votes", {
	...
	sessionId: int().notNull(),
	agentId: int().notNull(),
	decision: mysqlEnum(['approve','reject','abstain']).notNull(),
	confidence: decimal({ precision: 5, scale: 4 }).default('0.5').notNull(),
	reasoning: text(),
	latencyMs: int(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
}, (table) => [
	index("idx_council_votes_sessionId").on(table.sessionId),
	index("idx_council_votes_agentId").on(table.agentId),
]);
...
export const coupons = mysqlTable("coupons", {
	...
	validForBundleIds: json().$type<number[]>(),
	expiresAt: timestamp({ mode: 'string' }),
...
export const courseEnrollments = mysqlTable("course_enrollments", {
	...
	status: mysqlEnum(['enrolled','in_progress','completed','failed','suspended','cancelled']).default('enrolled').notNull(),
...
export const userEmailPreferences = mysqlTable("user_email_preferences", {
	id: int().autoincrement().notNull(),
	userId: int().notNull(),
	optIn: tinyint().default(1).notNull(),
	...
	productUpdates: tinyint().default(1).notNull(),
...
export const users = mysqlTable("users", {
	...
	name: text(),
	email: varchar({ length: 320 }),
	status: mysqlEnum(['active','suspended','inactive','pending']).default('active').notNull(),
...
export const giveawayApplications = mysqlTable("giveaway_applications", {
	...
	status: mysqlEnum(['pending', 'approved', 'rejected', 'enrolled', 'waitlisted']).default('pending').notNull(),
	confirmationEmailSent: tinyint().default(0).notNull(),
