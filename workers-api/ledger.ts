import type { Env, DecisionRecord } from '../types';
...
export async function handleLedger(request: Request, env: Env, path: string): Promise<Response> {
  ...
  // GET /api/ledger/:id — single record
  const recordMatch = path.match(/^\/api\/ledger\/(.+)$/);
  if (recordMatch && method === 'GET') {
    ...
    return Response.json({
      ...row,
      lower_bound: Boolean(row.lower_bound),
    });
  }
  ...
  if (path === '/api/ledger' && method === 'POST') {
    ...
    const id = (body as any).record_id || body.id || `DR-${String(Date.now()).slice(-4)}`;
  ...
  // GET /api/ledger/contested — open contradictions
  ...
  // GET /api/ledger/by-tag/:tag — records by tag
  ...
  // GET /api/ledger/stats — summary stats
